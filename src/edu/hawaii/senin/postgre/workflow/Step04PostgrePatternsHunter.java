 ...
 * 
 */
public class Step04PostgrePatternsHunter extends UCRGenericClassifier {
  ...
  //
  private static final int[][] params = { { 14, 5, 5, CLASSIC } };
  ...
  public static void main(String[] args) throws IndexOutOfBoundsException, TSException, IOException {
    ...
    for (int[] p : params) {
      ...
      // get a random series out
      //
      // double[] series1 = trainData.get("pre").get(0);
      // double[] series2 = trainData.get("post").get(0);
      // trainData.get("pre").remove(0);
      // trainData.get("post").remove(0);
      ...
      for (Entry<String, HashMap<String, Double>> e : tfidf.entrySet()) {
        ...
        for (int i = 0; i < MAX_WORDS_2_PRINT; i++) {
          ...
          System.out.println("\"" + pattern + "\", " + df.format(weight));
        }
      ...
      for (Entry<String, HashMap<String, Double>> e : tfidf.entrySet()) {
        ...
        List<double[]> testD = trainData.get(className);
        int seriesIdx = 0;
